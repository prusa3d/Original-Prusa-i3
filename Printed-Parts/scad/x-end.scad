...
// TR Lead Screw Nut trap
   // Hole for the nut
    translate(v=[0,-17, -1]) poly_cylinder(h = 9.01, r = 6.6, $fn = 25);

// TR Lead Screw Nut Screw Holes
for (n = [1 : 2])  //  Change 2 to 3 for 3 hole POM Nuts, also change below 180 to 120
{
    // Screw holes for TR nut
    translate(v=[0,-17, 0]) rotate([0, 0, (180*n)+45]) translate([0, 9.5, -1]) cylinder(h = 10, r = 1.55, $fn=25);
    // Nut traps for TR nut screws
    translate(v=[0,-17, 0]) rotate([0, 0, (180*n)+45]) translate([0, 9.5, 6]) rotate([0, 0, 0])cylinder(h = 3, r = 3.3, $fn=6);
}
