...
module bltouch_mount()
{
    difference()
    {
        translate([-31,-9,-26]) cube([14,4,26]); 
        
        // Screws
        translate([-25.235,-5,-22]) rotate([90,0,0]) cylinder( h=6, r=1.8, $fn=50 );     
        translate([-25.235,-5,-4]) rotate([90,0,0]) cylinder( h=6, r=1.8, $fn=50 ); 
        
        // _style_
        translate([-30.5,-10,-29]) rotate([0,-40,0]) cube([4,5,10]);
        translate([-24,-10,-29]) rotate([0,40,0]) cube([4,5,13]);
    }
}
...
module final_part()
{
    difference()
    {
        ...
        translate([-35,-29,-30]) rotate([45,0,0]) cube([30,10,10]);   
            ...
            //version
            translate([-8,-34,-0.4]) rotate([0,0,0]) linear_extrude(height = 0.6) 
            { text("R2-BL",font = "helvetica:style=Bold", size=4, center=true); }    
